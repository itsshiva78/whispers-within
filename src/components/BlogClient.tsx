      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-16 md:py-24 text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Explore articles about anonymous messaging, digital privacy, online safety, and creative ways to make the most of Whispers Within.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="space-y-6">
            {filteredArticles.length === 0 ? (
              <div className="text-center text-muted-foreground py-10">No articles found in this category.</div>
            ) : (
                ...
                <Link key={article.slug} href={`/blog/${article.slug}`} className="block group">
                  <article className="p-6 md:p-8 rounded-2xl transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl"
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold border ${colorMap[article.color] || 'bg-gray-500/10 text-gray-400'}`}>
                        {article.category}
                      </span>
